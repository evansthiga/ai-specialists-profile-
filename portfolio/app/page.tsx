    ...
    <div className="min-h-screen bg-background transition-colors duration-300">
      ...
      <header className="container px-4 py-8 md:py-12">
        ...
        <div className="flex flex-col items-center gap-4 text-center md:flex-row md:text-left">
          <div className="relative size-32 overflow-hidden rounded-full md:size-40 transition-transform duration-300 hover:scale-105 ring-2 ring-[hsl(var(--primary))] ring-offset-2 ring-offset-background">
            <Image
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl gradient-text">Evans Thiga</h1>
      ...
      <main className="container space-y-12 px-4 pb-12">
        ...
        <section className="mx-auto max-w-3xl space-y-4">
          <h2 className="text-2xl font-bold tracking-tight gradient-text">About Me</h2>
        ...
        {/* Skills Section */}
        <section className="mx-auto max-w-3xl space-y-4">
          <h2 className="text-2xl font-bold tracking-tight gradient-text">Skills</h2>
          <Skills />
        </section>

        {/* Projects Section */}
        <section className="space-y-4">
          <h2 className="text-2xl font-bold tracking-tight gradient-text">Completed Tasks</h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project, index) => (
              <Card
                key={index}
                className="animate-fadeIn card-hover flex flex-col"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <CardContent className="flex-1 p-6">
                  <h3 className="mb-2 font-semibold gradient-text">{project.title}</h3>
                  <p className="mb-4 text-sm text-muted-foreground">{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, tagIndex) => (
                      <span
                        key={tagIndex}
                        className="rounded-full bg-secondary/50 px-2.5 py-0.5 text-xs font-semibold text-secondary-foreground"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <CardFooter className="p-6 pt-0">
                  <Button asChild variant="outline" className="w-full hover:gradient-bg hover:text-white">
      ...
      <footer className="border-t">
        <div className="container flex flex-col items-center gap-4 py-8 md:flex-row md:justify-between">
          <h2 className="text-xl font-semibold gradient-text">Contact</h2>
          <div className="flex gap-4">
            <Link
              href="mailto:evans@example.com"
...
const projects = [
  ...
  },
  {
    ...
    description: "Designed and implemented an automated booking system using n8n workflows and AI integration.",
    tags: ["n8n", "Automation", "API Integration"],
    sampleUrl: "/samples/workflow",
  },
  {
